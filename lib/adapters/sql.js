...
var jsonConditionsToSQL = function(Model, conditions){
  ...
  for (var key in conditions){
    ...
    if (conditions && conditions.like) {
      for (var likeKey in conditions.like) {
        var value = conditions.like[likeKey];
        var clause = Model.schema[likeKey].like(value);
        // var clause = " LOWER(" + likeKey + ") LIKE '" + value + "' "
        clauses.push(clause);
      }

      // console.log(sqlStr.text, '<<<<<<<<<< sqlStr');
    }
  }
