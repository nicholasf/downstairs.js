  ...
  this.connection.query(sqlStr, function(err, results){
    ...
    if (results){
      for (var i in results.rows){
        var model = new self(results.rows[i]);
        models.push(model);
      }
    }
    cb(err, models);
...
Collection.find = function(conditions, cb){
  ...
  this.findAll(conditions, function(err, models){
    if (models && models[0]) {
      var data = models[0];

      if (queryEvents){
        self.emitQueryEvents(queryEvents, models[0]);
      }

      if (modelCallbacks){
        self.runModelCallbacks(modelCallbacks, models[0], cb);
      }
      else {
