...
// Collection.when = function(eventName, whenFunction){
//   this.prototype.on(eventName, whenFunction);
// }

// Collection.emitQueryEvents = function(queryEvents, data, cb){
//   var record = this;
...
//   queryEvents.forEach(eventEmitter);
// }
...
Collection.find = function(conditions, cb){
  ...
  var findCb = function(err, models){
    if (models && models[0]) {
      if (queryEvents){
        _self.emitQueryEvents(queryEvents, models[0]);
      }

      if (modelCallbacks){
        _self.runModelCallbacks(modelCallbacks, models[0], cb);
      }
      else {
        return cb(err, models[0]);
      }
