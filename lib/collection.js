 ...
 **
 ** Each of these functions calls the adapter that has been configured on the Model.
 **/
...
Collection.update = function(data, conditions, cb){
  this.configuration.adapter.udpate.apply(this, arguments);
}
...
Collection.getConnection = function(){
  if (!this.configuration.connection.url) {
    this.configuration.connection.url = this.Downstairs.url;
  }
