...
var createValidator = function(model, validationName){
  return function(cb){
    //a bit misleading but validationResult will hold a validation error message
    //necessary for using async.js to run through the entire series
    //of validations
    var validatorCb = function(err, result){
      if (err){
        var validationMessage = err;
        cb(validationMessage, null);
      }
    }
    model[validationName](validatorCb);
  }
}
...
Table.register = function(sql, validations){
  var Model = function(properties){
    ...
    this.validate = function(cb){
      ...
      async.parallel(validationCycle, cb);
    }
  };
